...
const Card = () => {
  ...
  const getData = async () => {
      try {
        ...
        setCards(prevCards => [...prevCards, transposedArray]);
        setToken(response.data.playcard_token);
        setArrCode(prevArrCode => [...prevArrCode, response.data.playcard_token]);
      } catch (err) {
          console.error(err);
      }
  };
      ...
      <div className="card">
        {cards.map((numbers, cardIndex) => (
              <div key={cardIndex} className="bingo-card"> 
                ...
                <div className="bingo-card-id">Card token: {token}</div>
              </div>
            ))}
          </div>
    </div>
